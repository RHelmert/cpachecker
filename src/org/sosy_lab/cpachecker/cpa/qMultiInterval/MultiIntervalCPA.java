...
public class MultiIntervalCPA extends AbstractCPA {
  ...
  @Option(
      secure = true,
      name = "runIntervalAnalysisOnly",
      toUppercase = false,
      description = "Runs only the intervalanalysis.")
  private boolean runIntervalAnalysisOnly = false;
  ...
  private MultiIntervalCPA(
    ...
    logger.log(
        Level.WARNING,
        "READ known Bugs section in MultiIntervalState.java class file or in the README");
  }


  @Override
  public AbstractState getInitialState(CFANode pNode, StateSpacePartition pPartition) {
    MultiIntervalState qstate = new MultiIntervalState(logger, maxLoops, runIntervalAnalysisOnly);
    return qstate;
  }

  @Override
  public TransferRelation getTransferRelation() {
    if (runIntervalAnalysisOnly) {
      // Only Intervals
      return new MultiIntervalRelation(logger, iMap, pCfa);
    } else {
      // Intervals and MinEntropy
      return new MultiIntervalRelation(logger, iMap, minmax, pCfa);
    }
  }
