 ...
 */
public class PInterval implements Comparable<PInterval> {
  public static final PInterval EMPTY = new PInterval(IntervalExt.EMPTY);
  public static final PInterval UNBOUND = new PInterval(IntervalExt.UNBOUND);
  public static final PInterval BOOLEAN_INTERVAL = new PInterval(IntervalExt.BOOLEAN_INTERVAL);
  public static final PInterval ZERO = new PInterval(IntervalExt.ZERO);
  public static final PInterval ONE = new PInterval(IntervalExt.ONE);
   ...
   */
  public PInterval(IntervalExt in) {
   ...
   */
  public PInterval(Long in, Long in2) {
  ...
  public void collapse() {
    ...
    for (int overstep = out.size() - maxOut; overstep > 0; overstep--) {
      BigInteger minlen = new BigInteger("" + Long.MAX_VALUE);
      IntervalExt low = null;
      for (IntervalExt i : out.descendingSet()) {
        if (i.size().compareTo(new BigInteger("" + minlen)) == -1) {
          minlen = i.size();
          low = i;
        }
      }
  ...
  public BigInteger size() {
    summarize();
    BigInteger size = in.size();
    for (IntervalExt i : out) {
      // Log.Log2("OldSize " + size + "sub" + i.size());
      size = size.subtract(i.size());
      // Log.Log2("newsize " + size);
    }
    return size;
  }
  ...
  public PInterval union(PInterval other) {
    // handle the in- Interval

    PInterval uni = new PInterval(in.union(other.in));
   ...
   */
  public PInterval intersect(PInterval other) {
    PInterval intersection = new PInterval(in.intersect(other.in));
   ...
   */
  public PInterval intersect(IntervalExt other) {
    PInterval intersection = new PInterval(in.intersect(other));
  ...
  @Override
  public int compareTo(PInterval r) {
  ...
  public boolean equals(Object other) {
    ...
    if (!(other instanceof PInterval)) {
      return false;
    }
    if (!(((PInterval) other).in.equals(this.in))) {
      return false;
    }

    if (!(((PInterval) other).size().equals(this.size()))) {
      return false;
    }
    if (((PInterval) other).out.size() != this.out.size()) {
      return false;
    }

    for (IntervalExt i : ((PInterval) other).out) {
      if (!(out.contains(i))) {
        return false;
      }
    }
   ...
   */
  public boolean isGreaterThan(PInterval other) {
  ...
  @Override
  public PInterval clone() {
    PInterval r = new PInterval(in);
    for (IntervalExt i : out) {
      r.addOut(i);
    }
    return r;
   ...
   */
  public PInterval plus(PInterval interval) {
   ...
   * @return a new Range with the respective bounds
   */
  public PInterval plus(IntervalExt interval) {
    if (isEmpty() || interval.isEmpty()) {
      return EMPTY;
    }
    PInterval rplus =
    ...
    // the same
    if (interval.size().equals(BigInteger.ONE)) {
      for (IntervalExt i : out) {
  ...
  /**
   * see {@link #plus(PInterval)} with the negation of the parameter
   *
   * @param other the other range to subtract
   * @return this range subtracted the other other range
   */
  public PInterval minus(PInterval other) {
   ...
   */
  public PInterval times(PInterval other) {
    Long[] values = {
   ...
   */
  public PInterval divide(PInterval other) {
   ...
   */
  public boolean contains(PInterval other) {
    ...
    PInterval temp = this.intersect(other);
   ...
   */
  public boolean maybeLessOrEqual(PInterval other) {
   ...
   */
  public PInterval limitUpperBound(PInterval other) {
    ...
    PInterval ret = new PInterval(getLow(), Math.min(getHigh(), other.getHigh()));
    for (IntervalExt i : out) {
      ret.addOut(i);
    }
    return ret;
   ...
   */
  public PInterval limitLowerBound(PInterval other) {
    ...
    PInterval ret = new PInterval(Math.max(getLow(), other.getLow()), getHigh());
    for (IntervalExt i : out) {
      ret.addOut(i);
    }
    return ret;
   ...
   */
  public PInterval negate() {
    PInterval nRange = new PInterval(in.negate());
   ...
   */
  public boolean intersects(PInterval pOther) {
  ...
  public static void main(String[] args) {
    ...
    PInterval r = new PInterval(new IntervalExt(0L, 100L));
    // PInterval q;
    ...
    r.collapse();
    Log.Log2(r);
    // q = r.intersect(new IntervalExt(0L, 200L));
    Log.Log2(r.size());
  }
}
